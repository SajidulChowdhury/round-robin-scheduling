...
struct node* newNode(struct process* process1)
{
...
void enqueue_arrival(struct process* process1)
{
    struct node* node = newNode(process1);
...
void enqueue_roundrobin(struct process* process1)
{
    struct node* node = newNode(process1);
...
char* dequeue_arrival()
{

   struct node* temp = pending_head;
    pending_head = pending_head->next;
   
    if(pending_head==NULL) pending_tail=NULL;
    char* process_num = temp->p->process_ID;
    printf("%s has been removed",temp->p->process_ID);
...
//    int process_count=1;
    ...
    pending_tail = malloc(sizeof(struct node));
    pending_head= NULL;
    pending_tail = NULL;
    while (fgets(line, sizeof(line), file)) {
        ...
        temp->process_ID=malloc(sizeof(50));
        strcpy(temp->process_ID,name);
        temp->arrival_time=atoi(arrival_time);
        temp->service_time=atoi(service_time);
        
        enqueue_arrival(temp);
...
int main()
{
    ...
    printf("%s %d %d \n ",current->p->process_ID,current->p->arrival_time,current->p->service_time);
   ...
   while(current != NULL){
    ...
    for( int i =0;i<pending_head->p->arrival_time;i++)
    {
        run_one_step();
    }
    ...
    while(1)
    {
        ...
        if(count==4 && head==NULL && pending_head ==NULL){
            printf("finished");
            break;
        }
   ...
   { 
       ...
       {
       if(pending_head->p->arrival_time==computer.time)
       {
        enqueue_roundrobin(pending_head->p);
       // printf("%s has been removed",pending_head->p->process_ID);
        pending_head=NULL;
        pending_tail=NULL;
        break;
        }
...
dequeue_arrival();  }
    ...
    if(computer.cores[i].p->service_time==0 || computer.cores[i].proc_time== 20)
        {
        remove_proc(i);
        }
}
     ...
     for(int i =0; i<4;i++)
    {
        ...
        if(computer.cores[i].busy==0 && head !=NULL)
        {
        sched_proc(head->p,i);
        if(head->next !=NULL) dequeue_roundrobin();
        else
        {
            head=NULL;
        tail= NULL;
...
dequeue_arrival();  }
    ...
    {
    if(computer.cores[i].busy==1){
    if(computer.cores[i].p->service_time==0 || computer.cores[i].proc_time== 20|| computer.cores[i].p->io==1)
        {
